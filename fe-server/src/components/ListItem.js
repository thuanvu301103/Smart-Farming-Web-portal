      ...
      <CardContent>
        <Grid container>
          ...
          <Grid item xs={9}>
            ...
            <Divider
              sx={{ flexGrow: 1, mt: 2, backgroundColor: "success.main" }}
            />
            <Grid container spacing={0} mt={1}>
              ...
              <Grid item xs={12} container alignItems="center">
                ...
                <Typography variant="caption" color="text.secondary" ml={1}>
                  {Array.isArray(item?.plant_type) &&
                  item.plant_type.length === 0
                    ? t("common.unknown")
              ...
              <Grid item xs={12} display="flex" justifyContent="flex-end">
                <Button
                  ...
                  startIcon={
                    <BookmarkBorderIcon
                      color={
                        isFavorite ? (isFavorite ? "script" : "text") : "text"
                      }
                    />
                  }
...
const UserListItem1 = ({
  ...
  return (
    <CardWrapper borderThickness="0px" borderSide="right" borderColor="success">
      <Box
        display="flex"
        alignItems="center"
        m={1}
        ...
        >
          ...
          {key === "create_script" ? (
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <DescriptionOutlinedIcon sx={{ mr: 1 }} /> {t("create")}{" "}
              {value.length} {t("script")}
            </Box>
          ) : key === "create_model" ? (
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <ModelTrainingOutlinedIcon sx={{ mr: 1 }} /> {t("create")}{" "}
              {value.length} {t("model")}
            </Box>
          ) : null}
          {/*Link*/}
          {value.map((val) => (
            <Link href={val} underline="hover" sx={{ ml: 4 }} color="info">
              {val}
            </Link>
          ))}
        </Box>
  ...
  return (
    <Box sx={{ bgcolor: "background.paper", m: 2 }}>
      <ListItem>
        ...
        <ListItemText primary={item?.name ? item.name : null} />
      </ListItem>
    </Box>
  );
};
